...
class TraderBot {
    ...
    onMessage(data) {
        ...
        switch (data.type) {
            ...
            case 'match':
                console.log(data)
                open = this.manager.getOpenOrder()
                if (open.id == data.order_id) {
                    this.log(`${data.size} ${data.product_id} @ ${data.price}.`, data.side)
                    // figure out how to track last filled when it's partially filled in 2 orders
    ...
    onClose() {
        ...
        const interval = setInterval(() => {
            if (!websocket.socket) {
                ...
                if (tries % 30 === 0) {
                    self.log(`Websocket could not reconnect after ${tries} attempts. Shutting down.`)
                    process.exit()
                }
                websocket.connect()
            }
    ...
    connectRealtimeFeed() {
        ...
        const heartbeatInterval = setInterval(() => {
            let now = new moment()
            if (now.diff(self.latestHeartbeat, 'seconds') >= maxSkippedBeats) {
                self.log('Websocket skipped a beat. Attempting to reconnect...')
    ...
    buy() {
        ...
        async function buyAsync(options) {
            try {
                ...
                this.log(`BUY order placed for ${options.size} ${options.product_id} @ $${options.price}`)
    ...
    sell() {
        ...
        async function sellAsync(options) {
            try {
                ...
                this.log(`SELL order placed for ${options.size} ${options.product_id} @ $${options.price}`)
                this.manager.setOpenOrder(data)
            } catch (err) {
                this.log(err, 'error')
            }
