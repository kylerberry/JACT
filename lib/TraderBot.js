...
class TraderBot {
    ...
    doneHandler(data) {
        ...
        if (data.reason == 'filled') {
            ...
            data.size = parseFloat(this.getLastReceived().size) - parseFloat(data.remaining_size)
            ...
            if (data.remaining_size !== '0') {
                console.log(`>> #${data.order_id}: ${data.side} for ${data.size} @ $${data.price} partially filled ${data.remaining_size}.`)
