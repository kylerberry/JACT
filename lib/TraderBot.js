...
// purchase strategy: buy at a price, when sell signal and price is above purchase price - wait, else sell.
// only sell in overbought territory when RSI and Stoch both indicate sell

/* 
or use 75-period (1 min candle) SMA:
- if sell signal and price below SMA, SELL
- if sell signal and price above SMA, only sell if RSI overbought
*/
class TraderBot {
    constructor({
        strategy,
    ...
    onMessage(data) {
        switch (data.type) {
            ...
            break
            case 'done':
                // if this is too slow I may need to poll the local orderbook
            ...
            case 'activate':
                ...
                if (stopOrder && stopOrder.id == data.order_id) {
                    ...
                    console.log(`>> Stop loss #${data.order_id} activated.\n`)
    ...
    cancelOrder(order) {
        ...
        async function cancelAsync(orderId) {
            console.log(`>> Canceling ${order.type} order #${orderId}.\n`)
            try {
                const data = await gdax.cancelOrder(options).catch(err => { throw new Error(err) })
                if (data.message) {
                    throw new Error(data.message)
                }
                //reset stop
